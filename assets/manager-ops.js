...
async function prepareDispatchModal() {
  ...
  try {
    ...
    if (rSel) {
      rSel.innerHTML = '<option value="">&mdash; Select route &mdash;</option>' +
    ...
    if (cSel) cSel.innerHTML = '<option value="">&mdash;</option>' + fieldUsers.map((u) => `<option value="${u.id}">${escMgr(u.full_name)}</option>`).join('');
