-- LAPOK DMS — EFRIS / fiscal device integration (fiscal-first import)
-- Run: mysql -u root lapok_dms < database/migrations/004_efris_integration.sql

USE lapok_dms;

-- ── Product mapping: Lapok product ↔ URA / fiscal device item code ─────
...
-- ── Imported fiscal receipts (device → Lapok) ─────────────────────────
...
-- ── Depot / device config (no secrets in repo — set in DB) ───────────
...
INSERT INTO efris_config (config_key, config_value, notes) VALUES
    ...
    ('default_device_serial', '', 'Primary field fiscal device — optional'),
    ('ingest_api_key', '', 'Shared secret for device push webhook — set in production')
