...
| **Done (2 Aug)** | **Live deployment + session security** | System **live on Truehost** at `dms.afriboards.com` (clean DB export; accounts + catalog + vehicles kept). Session idle timeout **5 min** (`SESSION_IDLE_TIMEOUT` in `.env`); open-tab **heartbeat every 30s** keeps active sessions alive; **10-min inactivity auto-logout**; `api/auth/me.php` enforces timeout on load. |
